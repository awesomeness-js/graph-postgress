...
describe('edge.search persistent date range debug logs', async () => {

	it('logs all records and between +/-5 day results', async () => {

		const seeded = await seedPersistentCreatedRecords();
		...
		console.log('between range:', { between: [ startDate, endDate ] });
		console.log('all persistent rows:', allPersistent.map((edge) => edge.properties));
		console.log('between rows:', betweenResults.map((edge) => edge.properties));
